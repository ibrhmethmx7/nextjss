...
function WatchContent() {
    ...
    useEffect(() => {
        ...
        if (initialUrl) {
            setQueue([{ id: "initial", title: decodeURIComponent(initialTitle) || "Video", url: decodeURIComponent(initialUrl), movieId: movieId || undefined }]);
        }
    ...
    const addToQueue = (item: any) => {
        setQueue([...queue, { id: Date.now().toString(), title: item.snippet.title, url: `https://www.youtube.com/watch?v=${item.id.videoId}`, thumbnail: item.snippet.thumbnails.default.url }]);
        setSearchResults([]);
        setSearchQuery("");
    };
        ...
        try {
            if (currentVideo?.movieId) {
                ...
                await set(push(ref(database, "movies")), {
                    title: currentVideo.title, poster: currentVideo.thumbnail || "", status: "completed",
                    videoUrl: currentVideo.url, addedAt: Date.now(), addedBy: currentUser,
                });
            }
            alert("Bitirdik!");
        } catch { alert("Hata!"); }
            ...
            <div className="min-h-screen bg-[#0a0a0a] text-white p-4">
                ...
                <div className="max-w-xl mx-auto mt-8 space-y-6">
                    ...
                    <div className="pt-4 border-t border-white/10">
                        ...
                        <Input placeholder="Video URL..." className="bg-white/5 border-white/10 h-12" style={{ fontSize: '16px' }} onKeyPress={(e) => { if (e.key === "Enter") { setQueue([{ id: Date.now().toString(), title: "Video", url: (e.target as HTMLInputElement).value }]); } }} />
                    </div>
                </div>
            </div>
        );
    ...
    // Main watch view - single layout for both normal and fullscreen
    return (
        <div ref={containerRef} className={`bg-[#0a0a0a] text-white flex flex-col ${isFullscreen ? 'fixed inset-0 z-50' : 'min-h-screen'}`}>
            ...
            {/* Video Container */}
            <div className={`relative ${isFullscreen ? 'flex-1' : ''}`} style={isFullscreen ? {} : { paddingTop: "56.25%" }}>
                <iframe
            ...
            {/* Bottom Panel - hide in fullscreen */}
            {!isFullscreen && (
                <div className="flex-1 border-t border-white/5 bg-black/50 flex flex-col" style={{ minHeight: 280 }}>
