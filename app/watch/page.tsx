                ...
                <div className={`${isFullscreen ? 'flex-1' : 'flex-1 md:grid md:grid-cols-[1fr_320px] lg:grid-cols-[1fr_384px] md:items-start md:gap-0 h-full'}`}>
                    ...
                    <div className={`relative w-full ${isFullscreen ? 'h-full' : 'h-full bg-black'}`}>
                        <div className="absolute inset-0">
                            ...
                            <div className="absolute inset-0 z-0" onClick={handleTogglePlay} />
                            <VideoControls
                                roomId={roomId}
                                isPlaying={isPlaying}
                                isMuted={isMuted}
                            ...
                            {isFullscreen && (
                                <div className="absolute left-0 right-0 bottom-20 px-4 z-[60] pointer-events-none">
                                    <div className="max-w-md ml-auto pointer-events-auto">
                                        ...
                                        <div className="flex gap-2" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()}>
                                            <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyPress={(e) => e.key === "Enter" && sendMessage()} onFocus={(e) => e.stopPropagation()} placeholder="Mesaj yaz..." className="bg-black/60 backdrop-blur border-white/20 h-9 text-sm" style={{ fontSize: '16px' }} />
                                            <Button size="sm" onClick={(e) => { e.stopPropagation(); sendMessage(); }} className="bg-red-600 hover:bg-red-700 h-9 px-3"><Send className="h-4 w-4" /></Button>
                                        </div>
                                    </div>
                                </div>
                            )}
