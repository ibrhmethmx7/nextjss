...
export default function SurpriseEffect({ roomId }: { roomId: string }) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(surpriseRef, (snapshot) => {
            ...
            // Only trigger if it's a new timestamp
            if (timestamp) {
                // First load: just sync the state, don't fire unless it's very new (< 10s)
                if (lastTriggeredRef.current === 0) {
                ...
                // Subsequent updates: ALWAYS fire if timestamp is newer
